...
import { IContainerRecord, ITouchStoneContainer } from './interfaces';
...
let search = async (index: number, size: number, searchKey: string) => {
    let response = await apis.runAsync("search", {
        index,
        size,
        searchKey,
    });
...
let main = async () => {
    ...
    let output = [] as IContainerRecord[];
    ...
    let searchResult = await search(1, 999, "");
