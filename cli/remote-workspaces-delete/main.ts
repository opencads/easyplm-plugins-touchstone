...
import { IWorkspaceRecord, MCADCatalog } from '../remote-workspaces-active/interfaces';
...
let deleteWorkspace = async (oid: string) => {
    let response = await apis.runAsync("delete", {
        oid
    });
...
let main = async () => {
    ...
    await deleteWorkspace(input.key);
    File.WriteAllText(outputPath, JSON.stringify(output), utf8);
};
