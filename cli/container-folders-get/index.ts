...
import { Folder, IFolderRecord } from './interfaces';
...
let searchTree = async (containerOid: string, containerModel: string) => {
    let response = await apis.runAsync("searchTree", {
        containerOid,
        containerModel
    });
...
let main = async () => {
    ...
    let input = Json.Load(inputPath) as IContainerRecord;
    let output = [] as IFolderRecord[];
    ...
    let searchResult = await searchTree(input.key, input.raw.modelDefinition);
